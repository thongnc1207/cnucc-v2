...
interface ButtonUploadImgProps {
  setPreviewUrl: (value: string[]) => void; // Changed to string array
  setUploadedImage: (value: string[]) => void; // Changed to string array
  ...
  const handleFileChange = async (
    ...
    if (files.length > 0) {
      try {
        ...
        let response;
        if (files.length > 1) {
          response = await uploadImage(files);
          const mediaArray = Array.isArray(response.data) ? response.data : [response.data];
          const mediaUrls = mediaArray.map((item: Media) => item.url);
          setUploadedImage(mediaUrls);
        } else {
          console.log("In image.tsx: "+ files);
